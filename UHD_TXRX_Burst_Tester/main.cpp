...
void rx_thread_function(SDR_Device_Wrapper::sptr_t sdr_device_wrapper, vector<complex<float>>& rx_buffer, int64_t rx_start_tick, int64_t no_of_ticks_per_bursts_period, bool rx_active)
{
	try
	{
...
int main(int argc, char *argv[])
{
	...
	sdr_device_wrapper->get_device()->set_time_now(uhd::time_spec_t(0.0f),0);

	int64_t now_tick = sdr_device_wrapper->get_device()->get_time_now().to_ticks(device_cfg->f_clk);

	int64_t tx_start_tick = now_tick + uhd::time_spec_t(1 + burst_period).to_ticks(device_cfg->f_clk);
	int64_t rx_start_tick = now_tick + uhd::time_spec_t(1 + burst_period + rx_tx_separation).to_ticks(device_cfg->f_clk);
